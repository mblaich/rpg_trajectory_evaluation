...
def alignSE3horn(p_es, p_gt, q_es, q_gt, n_aligned=-1):
    '''
    Calculate SE3 transformation R and t so that:
        gt = R * est + t
    '''
    if n_aligned == 1:
        ...
        est_pos = p_es[idxs, 0:3]
        gt_pos = p_gt[idxs, 0:3]
        s, R, t = align.align_horn(gt_pos, est_pos)  # note the order
        R, t, error =  align.align_horn(gt_pos, est_pos)
...
def alignTrajectory(p_es, p_gt, q_es, q_gt, method, n_aligned=-1):
    ...
    if method == 'sim3':
        ...
        R, t = alignSE3horn(p_es, p_gt, q_es, q_gt, n_aligned)
    elif method == 'none':
        R = np.identity(3)
        t = np.zeros((3, ))
    else:
